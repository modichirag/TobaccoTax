...
def get_estimate(z0, z1, z2, constraint=True, bound=True):
    j = z0.shape[0]                                                                                                                               
    ...
    chisq = lambda w, x, y: ((y - np.dot(w, x))**2).sum()
...
def fit(controls, treated, T0, constraint=True, bound=True):
    '''
    ...
    return  get_estimate(Z0, Z1, Z2, constraint, bound)  
