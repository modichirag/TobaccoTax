...
class GLearn():
    ...
    def __init__(self, controls, padl=0, padr=0, ptransform=None, p0=None, n0=0.01, ninf=1e10, stdscale=False, seed=100):
        ...
        self.ffpad = np.fft.rfftfreq(self.T + self.padl + self.padr)
        #
        if ptransform is None: self.ptransform = yjt.get_transform(controls, stdscale=stdscale, p0=p0) 
        else: self.ptransform = ptransform
        #  order in ptransform : mmean, sstd, mu, sig, eta, eps, beta
        self.controlsg = yjt.transform(self.controls.copy(), self.ptransform) 
        self.getprior()
        #
    ...
    def fit(self, treated, T0, sigma=None, ninf=1e10):
        ...
        treatedg = yjt.transform(treated, self.ptransform).flatten() #
        ...
        def _neglogposterior(p):
            ...
            x = np.fft.irfft(u + 1j*v, norm='ortho')
            ps = abs(u + 1j*v)**2
        ...
        predg = np.fft.irfft(pp[:pp.size//2] + 1j*pp[pp.size//2:], norm='ortho')
        predg = predg[self.padl:-self.padr]
        ...
        err = (errg[self.padl:-self.padr]*deriv)*self.ptransform[1]
        #
    ...
    def getcov(self, T0, n0=0.01, ninf=1e10, real=True):
        ...
        if real: psf = np.concatenate([psf, psf[1:-1][::-1]])
